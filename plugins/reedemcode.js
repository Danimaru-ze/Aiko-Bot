...
let handler = async (m, { conn, args }) => {
    ...
    if (fs.existsSync(`./assets/database/userclaim/${m.sender}.json`)) return m.reply('Anda Telah Claim Kode Hari Ini Tunggu Besok hari')
    ...
    let data;
    try {
        data = JSON.parse(fs.readFileSync("./assets/database/codereedem.json"));
    } catch (error) {
    ...
    if (data.includes(args[0])) {
        ...
        data.splice(unp, 1)
        fs.writeFileSync("./assets/database/codereedem.json", JSON.stringify(data))
    } else {
