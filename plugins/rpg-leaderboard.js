...
${usedPrefix}${command} legendary`.trim()
    if (!leaderboard.includes(type)) return conn.adReply(m.chat, '*––––『 𝙻𝙴𝙰𝙳𝙴𝚁𝙱𝙾𝙰𝚁𝙳 』––––*\n' + wrong, 'L E A D E R B O A R D', '', flaImg.getRandom() + 'LEADERBOARD', global.link.website, m)
    let page = isNumber(args[1]) ? Math.min(Math.max(parseInt(args[1]), 0), getPage(type)): 0
    let sortedItem = users.map(toNumber(type)).sort(sort(type))
    let userItem = sortedItem.map(enumGetKey)
    const pp = who => conn.profilePictureUrl(who, 'image').catch(_ => fs.readFileSync('./assets/src/avatar_contact.png'));
