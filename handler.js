  ...
  try {
    ...
    try {
      ...
      let info = {
        ...
        wm: bot.watermark,
        author: owner.name,
        chtext: 'info tentang bot Aiko!',
        sambutan: owner.name+' Hallo!',
      }
      let invest = {
        cooldown: 0,
        item: {
    ...
    const isMods = global.owner.access.filter(([number, _, isDeveloper]) => number && isDeveloper).map(([number]) => number).map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender) || global.db.data.bots.owner.filter(([number, _, isDeveloper]) => number && isDeveloper).map(([number]) => number).map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
    const isOwner = m.fromMe || isMods || global.owner.access.filter(([number, _, isDeveloper]) => number && !isDeveloper).map(([number]) => number).map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
    ...
    for (let name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          console.error(e)
          for (let [jid] of global.owner.access.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
     ...
     /*if (opts && match && m) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (plugin.level > _user.level) {
          this.adReply(m.chat, `ʟᴇᴠᴇʟ ᴋᴀᴍᴜ ʜᴀʀᴜs ᴅɪ ᴀᴛᴀs ${plugin.level} ᴜɴᴛᴜᴋ ʙɪsᴀ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ`, 'ＡＫＳＥＳ ＤＩＴＯＬＡＫ', global.bot.watermark, fs.readFileSync('./assets/media/denied.jpg'), global.link.group, m, false)
          continue
        }
        if (plugin.age > _user.age) {
          this.adReply(m.chat, `ᴜᴍᴜʀ ᴋᴀᴍᴜ ʜᴀʀᴜs ᴅɪ ᴀᴛᴀs ${plugin.age} ᴜɴᴛᴜᴋ ʙɪsᴀ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ`, 'ＡＫＳＥＳ ＤＩＴＯＬＡＫ', global.bot.watermark, fs.readFileSync('./assets/media/denied.jpg'), global.link.saluran, m, false)
          continue
        }
        let extra = {
          match,
        ...
        try {
          ...
          if (e) {
            let text = format(e)
            for (let key of Object.values(global.API.APIKeys))
    ...
    case 'add': {
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          let pp = await this.profilePictureUrl(user, 'image').catch(_ => fs.readFileSync('./assets/src/avatar_contact.png'))
...
global.dfail = (type, m, conn) => {
  let msg = {
    owner: global.message.owner || '*ᴏᴡɴᴇʀ ᴏɴʟʏ •* ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ʜᴀɴʏᴀ ᴜɴᴛᴜᴋ ᴏᴡɴᴇʀ ʙᴏᴛ !!',
    mods: global.message.mods || '*ᴅᴇᴠᴇʟᴏᴘᴇʀ ᴏɴʟʏ •* ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ʜᴀɴʏᴀ ᴜɴᴛᴜᴋ ᴅᴇᴠᴇʟᴏᴘᴇʀ ʙᴏᴛ !!',
    premium: global.message.premium || '*ᴘʀᴇᴍɪᴜᴍ ᴏɴʟʏ •* ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ʜᴀɴʏᴀ ᴜɴᴛᴜᴋ ᴍᴇᴍʙᴇʀ ᴘʀᴇᴍɪᴜᴍ !!',
    group: global.message.group || '*ɢʀᴏᴜᴘ ᴏɴʟʏ •* ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ʜᴀɴʏᴀ ᴅᴀᴘᴀᴛ ᴅɪɢᴜɴᴀᴋᴀɴ ᴅɪ ɢʀᴏᴜᴘ !!',
    private: global.message.private || '*ᴘʀɪᴠᴀᴛᴇ ᴏɴʟʏ •* ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ʜᴀɴʏᴀ ᴅᴀᴘᴀᴛ ᴅɪɢᴜɴᴀᴋᴀɴ ᴅɪ ᴄʜᴀᴛ ᴘʀɪʙᴀᴅɪ !!',
    admin: global.message.admin || '*ᴀᴅᴍɪɴ ᴏɴʟʏ •* ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ʜᴀɴʏᴀ ᴜɴᴛᴜᴋ ᴀᴅᴍɪɴ ɢʀᴏᴜᴘ !!',
    botAdmin: global.message.botAdmin || 'ᴊᴀᴅɪᴋᴀɴ ʙᴏᴛ ꜱᴇʙᴀɢᴀɪ ᴀᴅᴍɪɴ ᴜɴᴛᴜᴋ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ',
    segel: global.message.segel || 'ᴍᴀᴀꜰ ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ᴛɪᴅᴀᴋ ʙɪꜱᴀ ᴅɪɢᴜɴᴀᴋᴀɴ ᴋᴀʀɴᴀ ʀᴀᴡᴀɴ ʙᴀɴɴᴇᴅ !!',
    onlyprem: global.message.onlyprem || 'ʜᴀɴʏᴀ ᴜꜱᴇʀ *ᴘʀᴇᴍɪᴜᴍ* ʏᴀɴɢ ᴅᴀᴘᴀᴛ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ꜰɪᴛᴜʀ ɪɴɪ ᴅɪ *ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ* !!',
    nsfw: global.message.nsfw || 'ᴀᴅᴍɪɴ ᴍᴇɴᴏɴᴀᴋᴛɪғᴋᴀɴ ғɪᴛᴜʀ *ɴsғᴡ* ᴅɪ ɢʀᴏᴜᴘ ɪɴɪ!',
    rpg: global.message.rpg || 'ᴀᴅᴍɪɴ ᴍᴇɴᴏɴᴀᴋᴛɪғᴋᴀɴ ғɪᴛᴜʀ *ʀᴘɢ ɢᴀᴍᴇ* ᴅɪ ɢʀᴏᴜᴘ ɪɴɪ!',
    game: global.message.game || 'ᴀᴅᴍɪɴ ᴍᴇɴᴏɴᴀᴋᴛɪғᴋᴀɴ ғɪᴛᴜʀ *ɢᴀᴍᴇ* ᴅɪ ɢʀᴏᴜᴘ ɪɴɪ!',
    limitExp: global.message.limitExp || 'ʟɪᴍɪᴛ ᴋᴀᴍᴜ ᴛᴇʟᴀʜ ʜᴀʙɪꜱ ʙᴇʙᴇʀᴀᴘᴀ ᴄᴏᴍᴍᴀɴᴅ ᴛɪᴅᴀᴋ ᴅᴀᴘᴀᴛ ᴅɪᴀᴋꜱᴇꜱ! \nᴜɴᴛᴜᴋ ᴍᴇɴᴅᴀᴘᴀᴛᴋᴀɴ ʟɪᴍɪᴛ ᴀɴᴅᴀ ʙɪꜱᴀ ᴍᴇᴍʙᴇʟɪɴʏᴀ ᴅᴇɴɢᴀɴ *#ʙᴜʏ ʟɪᴍɪᴛ* ᴀᴛᴀᴜ ᴍᴇɴᴜɴɢɢᴜ ʟɪᴍɪᴛ ʀᴇꜰʀᴇꜱʜ ꜱᴇᴛɪᴀᴘ ʜᴀʀɪ.',
    restrict: global.message.restrict || 'ꜰɪᴛᴜʀ ɪɴɪ ᴛɪᴅᴀᴋ ᴅᴀᴘᴀᴛ ᴅɪɢᴜɴᴀᴋᴀɴ !!',
    unreg: global.message.unreg || 'sɪʟᴀʜᴋᴀɴ ᴅᴀғᴛᴀʀ ᴋᴇ *ᴅᴀᴛᴀʙᴀsᴇ* ʙᴏᴛ ᴛᴇʀʟᴇʙɪʜ ᴅᴀʜᴜʟᴜ ᴊɪᴋᴀ ɪɴɢɪɴ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ғɪᴛᴜʀ ɪɴɪ!\n\nᴄᴏɴᴛᴏʜ:\n#ᴅᴀғᴛᴀʀ ɴᴀᴍᴀᴍᴜ.ᴜᴍᴜʀᴍᴜ'
  } [type]
  if (msg) return conn.adReply(m.chat, msg, 'ＡＫＳＥＳ ＤＩＴＯＬＡＫ', global.bot.watermark, fs.readFileSync('./assets/media/denied.jpg'), global.link.group, m, true)
}
